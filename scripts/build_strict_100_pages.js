/**
 * ============================================================================
 * FARM CENTRAL — STRICT 100-PAGE COLLEGE PROJECT REPORT BUILDER
 * ============================================================================
 * Structure:
 * - Pages 1–3: Exact Cover, Certificate, and Acknowledgement from reference PDF.
 * - Pages 4–7: Preliminary Pages (TOC Part 1, TOC Part 2, List of Figures, Abstract).
 * - Pages 8–100 (93 Pages): Chapters 1 to 10 covering Introduction, Analysis, SRS,
 *   Tech Stack, System Design & UML, Implementation & Code, Testing, UI Screens,
...
// Page definitions: array of 97 functions, each rendering exactly one page
const pages = [];

// Helper to wrap single page rendering
function definePage(isPrelim, label, renderFn) {
    pages.push({ isPrelim, label, renderFn });
}

// ----------------------------------------------------------------------------
// PRELIMINARY PAGES (4 to 7 -> Roman IV to VII)
// ----------------------------------------------------------------------------

// Page 4 (IV): Table of Contents Part 1
definePage(true, 'IV', (doc) => {
...
// Page 5 (V): Table of Contents Part 2
definePage(true, 'V', (doc) => {
    ...
    drawTable(doc, ['CHAPTER', 'TITLE / TOPIC', 'PAGE NO'], [
        ...
        ['4.2', 'Express.js RESTful API Routing & Middleware', '21'],
        ...
        ['4.7', 'Leaflet.js Geospatial & Chart.js Visual Engines', '29'],
        ['5', 'SYSTEM DESIGN & ARCHITECTURAL MODELING', '30 – 43'],
        ['5.1', 'High-Level System Architecture & Component Diagram', '30'],
        ['5.2', 'UML Use Case Modeling & Scenario Descriptions', '32'],
        ['5.3', 'UML Class Diagram & Domain Entities', '34'],
        ...
        ['6.1 – 6.25', 'Modular Source Code & Algorithm Implementations', '44 – 68'],
...
// Page 6 (VI): List of Figures & Tables
definePage(true, 'VI', (doc) => {
...
// Loop to build all 93 main pages
for (let pIdx = 1; pIdx <= 93; pIdx++) {
    const arabPage = pIdx;
    
    definePage(false, `Page ${arabPage}`, (doc) => {
        ...
        if (arabPage === 1) {
            ...
            drawBullet(doc, 'Smallholder & Commercial Farmers', 'Access field monitoring, crop diagnosis, task scheduling, expense tracking, and market listings.');
            ...
            drawBullet(doc, 'Agronomists & Extension Officers', 'Monitor regional disease outbreaks, broadcast advisories, and verify farmer KYC credentials.');
            drawBullet(doc, 'Platform Administrators', 'Manage system security, audit transactions, oversee dispute resolution, and analyze platform telemetry.');
        ...
        // CHAPTER 3: Pages 12 to 18
            ...
            drawP(doc, '• High Availability & Fault Tolerance: The dual-adapter database architecture automatically falls back to local SQLite if cloud MySQL experiences network disconnection.');
        } else if (arabPage === 17) {
        ...
        // CHAPTER 7: Pages 69 to 78 (Testing - 10 pages)
            ...
            if (testPage === 1) {
                ...
                drawBullet(doc, 'User Acceptance Testing (UAT)', 'Validates workflow completion by agricultural growers and commercial produce buyers.');
            } else if (testPage === 2) {
...
console.log(`Total defined pages to generate: ${pages.length} pages (Pages 4 to 100).`);
...
async function generateSinglePageBuffer(pageDef) {
    return new Promise((resolve) => {
        ...
        doc.text('SRI VASAVI INSTITUTE OF ENGINEERING & TECHNOLOGY', 60, 26);
        doc.fontSize(8).font('Helvetica').fillColor('#64748b');
        doc.text('Department of Computer Engineering', width - 230, 26, { align: 'right', width: 175 });
        doc.strokeColor('#cbd5e1').lineWidth(0.6).moveTo(60, 38).lineTo(width - 55, 38).stroke();

        // Footer
        doc.moveTo(60, height - 38).lineTo(width - 55, height - 38).stroke();
        doc.fontSize(8).font('Helvetica').fillColor('#64748b');
        doc.text('Farm Central — Agricultural Intelligence & Telemetry Platform', 60, height - 30);
        doc.font('Helvetica-Bold').fillColor('#0f172a');
        doc.text(pageDef.label, width - 150, height - 30, { align: 'right', width: 95 });
        doc.restore();
...
async function buildStrict100Pages() {
    console.log(`Rendering ${pages.length} individual chapter pages...`);
    ...
    console.log('✅ Appended exact First 3 Pages from reference PDF.');

    // 2. Render each of the 97 pages and append page 0
    for (let i = 0; i < pages.length; i++) {
        const pageBuf = await generateSinglePageBuffer(pages[i]);
    ...
    console.log(`\n======================================================`);
    console.log(`📄 FINAL DOCUMENT PAGE COUNT: ${totalPages} PAGES`);
    console.log(`======================================================\n`);
